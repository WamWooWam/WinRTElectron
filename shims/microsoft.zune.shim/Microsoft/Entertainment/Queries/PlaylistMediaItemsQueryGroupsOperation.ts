// --------------------------------------------------
// <auto-generated>
//     This code was generated by tswinrt.
//     Generated from Microsoft.Entertainment 255.255.255.255 at Mon Mar 29 16:23:03 2021
// </auto-generated>
// --------------------------------------------------

import { IPlaylistMediaItemsQuery } from "./IPlaylistMediaItemsQuery";
import { IPlaylistMediaItemsQueryOperation } from "./IPlaylistMediaItemsQueryOperation";
import { IQueryResult } from "./IQueryResult";
import { MediaDataset } from "./MediaDataset";
import { AsyncOperationProgressHandler } from "winrt/Windows/Foundation/AsyncOperationProgressHandler`2";
import { AsyncOperationWithProgressCompletedHandler } from "winrt/Windows/Foundation/AsyncOperationWithProgressCompletedHandler`2";
import { AsyncStatus } from "winrt/Windows/Foundation/AsyncStatus";
import { HResult } from "winrt/Windows/Foundation/HResult";
import { IAsyncInfo } from "winrt/Windows/Foundation/IAsyncInfo";
import { IAsyncOperationWithProgress } from "winrt/Windows/Foundation/IAsyncOperationWithProgress`2";
import { GenerateShim } from "winrt/Windows/Foundation/Interop/GenerateShim";

@GenerateShim('Microsoft.Entertainment.Queries.PlaylistMediaItemsQueryGroupsOperation')
export class PlaylistMediaItemsQueryGroupsOperation implements IPlaylistMediaItemsQueryOperation, IAsyncOperationWithProgress<IQueryResult, number>, IAsyncInfo { 
    progress: AsyncOperationProgressHandler<IQueryResult, number> = null;
    completed: AsyncOperationWithProgressCompletedHandler<IQueryResult, number> = null;
    query: IPlaylistMediaItemsQuery = null;
    errorCode: number = null;
    id: number = null;
    status: AsyncStatus = null;
    wait(): MediaDataset {
        throw new Error('PlaylistMediaItemsQueryGroupsOperation#wait not implemented')
    }
    getResults(): IQueryResult {
        throw new Error('PlaylistMediaItemsQueryGroupsOperation#getResults not implemented')
    }
    cancel(): void {
        console.warn('PlaylistMediaItemsQueryGroupsOperation#cancel not implemented')
    }
    close(): void {
        console.warn('PlaylistMediaItemsQueryGroupsOperation#close not implemented')
    }
}
