...
@GenerateShim('Microsoft.Media.Advertising.DocumentAdPayloadHandler')
export class DocumentAdPayloadHandler implements IAdPayloadHandler, IStringable { 
    static readonly adType: string = null;
    readonly supportedTypes: string[] = null;
    player: IPlayer = null;
    cancelAd(force: boolean): IAsyncOperation<boolean> {
        return AsyncOperation.from(async () => { throw new Error('DocumentAdPayloadHandler#cancelAd not implemented') });
    }
    playAdAsync(adSource: IAdSource, startTimeout: number | null): IAsyncActionWithProgress<AdStatus> {
        return AsyncActionWithProgress.from(async () => console.warn('DocumentAdPayloadHandler#playAdAsync not implemented'));
    }
    preloadAdAsync(adSource: IAdSource): IAsyncAction {
        return AsyncAction.from(async () => console.warn('DocumentAdPayloadHandler#preloadAdAsync not implemented'));
    }
    toString(): string {
        throw new Error('DocumentAdPayloadHandler#toString not implemented')
    }
