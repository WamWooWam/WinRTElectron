...
export class TileGroup {
    ...
    constructor(tileGroupElement: HTMLDivElement) {
        ...
        for (let i = 0; i < tileElements.length; i++) {
            ...
            this.tiles.push(TileFactory.createTile(tileElement, tileGroupElement));
        }
    }
    ...
    layout() {
        let availableHeight = this.startTilesElement.offsetHeight - this.headerText.offsetHeight + 32;
        let maxRows = Math.floor((Math.floor(availableHeight / 64) - 1) / 2) * 2;
