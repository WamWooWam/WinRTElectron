    ...
    WinJS.Namespace.define("MS.Entertainment.Navigation", {
        BackStack: MS.Entertainment.defineObservable(function backStackConstructor() {
            ...
            StackEntry: WinJS.Class.define(function stackEntryConstructor(page, hub, panel) {
                ...
                _ia: null, _backStack: null, _clearBackStackOnNextNavigate: false, _domUpdatesEnabled: true, _navigationFocusTimeout: 5000, _maxEnterAnimationTimeout: 20000, canNavigateBack: MS.Entertainment.UI.Framework.observableProperty("canNavigateBack", false), overrideHideBackButton: MS.Entertainment.UI.Framework.observableProperty("overrideHideBackButton", false), navigationDirection: MS.Entertainment.UI.Framework.observableProperty("navigationDirection", MS.Entertainment.Navigation.NavigationDirection.forward), hasBackStack: MS.Entertainment.UI.Framework.observableProperty("hasBackStack", false), currentPage: MS.Entertainment.UI.Framework.observableProperty("currentPage", null, true), currentHub: MS.Entertainment.UI.Framework.observableProperty("currentHub", null), currentPanel: MS.Entertainment.UI.Framework.observableProperty("currentPanel", null), cachedPreviousLocationTelemetry: null, lastNavigationWasPage: MS.Entertainment.UI.Framework.observableProperty("lastNavigationWasPage", false), skipEnterAnimationOnNextNavigation: false, forceExitToSystemOnBack: false, _callOnNavigatingAway: function _callOnNavigatingAway(previous, next) {
                        if (previous) {
                                        ...
                                        if ((cacheOperation === MS.Entertainment.Navigation.Navigation.DOMCacheOperation.removeAllPages) && !newPage) {
                                            ...
                                            var fragmentCompleted = MS.Entertainment.Utilities.loadHtmlPage(fragmentUrl, target).then(function navigationFragmentLoaded() {
                                                    ...
                                                    return MS.Entertainment.Utilities.enterElement(target).then(function navigationPageEntered() {
                                                            ...
                                                            if (this.cachedPreviousLocationTelemetry)
                                                                previousLocationTelemetry = this.cachedPreviousLocationTelemetry;
                                                            else if (oldPage && oldPage.iaNode)
                                                                previousLocationTelemetry = oldPage.iaNode.moniker;
                                                            var currentLocationTelemetry = this.getUserLocation();
                                                            this.cachedPreviousLocationTelemetry = currentLocationTelemetry;
                        ...
                        return this.buildUserLocation(this.currentPage, this.currentHub, this.currentPanel)
                    }, buildUserLocation: function buildUserLocation(page, hub, panel) {
                        var userLocation = null;
                        if (page) {
                            userLocation = WinJS.Binding.unwrap(page).iaNode.moniker;
                            if (hub) {
                                userLocation = userLocation.concat("/", WinJS.Binding.unwrap(hub).iaNode.moniker);
                                if (panel)
                                    userLocation = userLocation.concat("/", WinJS.Binding.unwrap(panel).iaNode.moniker)
                            }
                        }
                        else
