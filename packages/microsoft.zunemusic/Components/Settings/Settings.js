...
(function() {
    ...
    WinJS.Namespace.define("MS.Entertainment.Utilities", {PreferenceSettingsWrapper: MS.Entertainment.UI.Framework.deriveUserControl("MS.Entertainment.Utilities.SettingsWrapper", null, function(element, options){}, {
            _onIsGrovelingChangedHandler: null, _displayingCredUI: false, _signIn: null, _sessionMgr: null, _configManager: null, initialize: function initialize() {
                    ...
                    this._initializeOneDriveDedupeSetting();
                    ...
                    e.stopPropagation();
                    e.preventDefault()
                }, showResetCloudCollectionDialog: function showResetCloudCollectionDialog() {
                    window.focus();
                    MS.Entertainment.Music.ResetCloudCollectionDialog.showDialog()
                }, handleKeyDownForResetCloudCollectionDialog: function handleKeyDownForResetCloudCollectionDialog(e) {
                    if (e.keyCode !== WinJS.Utilities.Key.enter)
                        return;
                    this.showResetCloudCollectionDialog();
                    e.stopPropagation();
                    e.preventDefault()
                ...
                }, oneDriveDedupeToggleChange: function oneDriveDedupeToggleChange(event) {
                    var cloudCollectionService = this._getCloudCollectionService();
                    if (cloudCollectionService)
                        if (this.oneDriveDedupe.checked)
                    ...
                    if (MS.Entertainment.Utilities.isMusicApp && cloudCollectionService && cloudCollectionService.isAvailable && !cloudCollectionService.isV2Enabled && this._signIn.isSignedIn) {
                        ...
                        if (signedInUser && !signedInUser.isSubscription)
                            this._hideElement(this.cloudMatchingOptInLink)
                    }
                    else {
                    ...
                    if (!cloudCollectionService.isEnabled || !this._signIn.isSignedIn)
                        this._hideElement(this.resetCloudCollectionWebLink);
                    this._updateMusicDownloadSettingsContainer()
                ...
                }, _initializeOneDriveDedupeSetting: function _initializeOneDriveDedupeSetting() {
                    var cloudCollectionService = this._getCloudCollectionService();
                    if (cloudCollectionService && cloudCollectionService.isV2Enabled && cloudCollectionService.isDeduplicationUIEnabled && this._signIn.isSignedIn) {
                        if (this.oneDriveDedupe) {
                            this.oneDriveDedupe.title = String.load(String.id.IDS_SETTINGS_ONEDRIVE_DEDUP_SETTING_TITLE);
                            this.oneDriveDedupe.checked = cloudCollectionService.isDeduplicationStrategyActive;
                            this.oneDriveDedupe.addEventListener("change", this.oneDriveDedupeToggleChange.bind(this))
                        }
                        if (this.oneDriveDedupeToggleContainer)
                            this.oneDriveDedupeToggleContainer.title = String.load(String.id.IDS_SETTINGS_ONEDRIVE_DEDUP_SETTING_SUBTITLE)
                    }
                    else
                        this._hideElement(this.oneDrivePreferencesContainer)
                    ...
                    this.oneDriveDedupe.removeEventListener("change", this.oneDriveDedupeToggleChange);
