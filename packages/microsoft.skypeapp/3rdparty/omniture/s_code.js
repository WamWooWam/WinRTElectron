...
if ((window.location.protocol === "file:") &&
  ((typeof s_allowLocal === "undefined") ||
    ((typeof s_allowLocal !== "undefined") &&
      (s_allowLocal !== true)))) {
  throw "[s_code.js]: do not track local copy";
}

var s_account = s_account || "skypeglobaldev";
if (location.hostname.indexOf(".skype.com") !== -1) {
  s_account = "skypeglobalprod";
}
...
s.linkInternalFilters = "javascript:,skype.com,skype.net" + "," + window.location.hostname;
...
function s_doPlugins(s) {
  ...
  try {
    if (s.campaign.toUpperCase().indexOf('AFCJ') === 0) {
      s.eVar34 = "D=v0";
    }
  } catch (e) { }
  ...
  if (!s.eVar35) {
    s.eVar35 = s.getQueryParam('intsrc');
    s.eVar35 = s.getValOnce(s.eVar35, 'eVar35', 0);
  }

  s.visitorID = s.visitorID || s.getCookie("skype_s_vi") || "";
  s.prop31 = (s.visitorID && "D=vid") || "";
  s.prop30 = s.prop30 || (((s.getCookie("s_vi") || "").match(/\|([^\[]*)/) || [])[1] || "");
  if (s.linkTrackVars !== "") {
    s.linkTrackVars = s.linkTrackVars + ",visitorID,prop31,prop30";
  }

  s.eVar7 = (s.pageName && "D=pageName") || "";
  s.eVar15 = (s.channel && "D=ch") || "";

  s.prop38 = s.timeFormat(new Date());
  s.prop39 = s.dateFormat(new Date());

  try {
    s.detectRIA('s_ria', 'eVar40');
  } catch (e) { }
  s.prop40 = (s.eVar40 && "D=v40");

  try {
    s.eVar6 = "";
    s.eVar6 = s.getCookie("SC").match(/VER=([^:]*)/)[1];
  } catch (e) { }

  if (s.eVar6 === "") {
    if (s.eVar40 === "flash not detected") {
      s.eVar6 = s.eVar18 = "data not available";
    } else {
      s.eVar6 = "non-user";
      s.eVar18 = "new";
    }
  ...
  s.prop24 = (s.eVar6 && "D=v6");
  s.prop21 = (s.eVar18 && "D=v18");

  s.hier1 = s.hier1 || (s.pageName || "").replace(/\//g, ",");
  s.server = s.server || location.hostname;

  s.prop23 = s.getPreviousValue(s.pageName, 'gpv_p23', '');
  s.eVar27 = (s.prop23 && "D=c23");


  s.prop37 = (s.prop36 && "D=g") || "";

  s.eVar5 = (s.prop5 && "D=c5");

  s.eVar54 = (s.prop54 && "D=c54");

  s.prop27 = "20121030|H.25.";

  try {
    var a = s.getCookie("SC_A"),
      b = s.getCookie("SC_B"),
      c = s.getCookie("SC_C");
    if (a === "1" && b === "1") { s.eVar74 = "beta site - testing: " + c; }
    if (a === "1" && b === "0") { s.eVar74 = "beta site - standard: " + c; }
    if (a === "0" && b === "1") { s.eVar74 = "current site - testing: " + c; }
    if (a === "0" && b === "0") { s.eVar74 = "current site - standard: " + c; }
    s.prop35 = (s.eVar74 && "D=v74");
    if (s.linkTrackVars !== "") {
      s.linkTrackVars = s.linkTrackVars + ",prop35,eVar74";
    }
  } catch (e) { }


  try {
    s.pageURL = (s.wd.location.href || "").toLowerCase();
    s.pageURL = s.pageURL.replace(/(username=[^&]*|skype-session=[^&]*)/g, "mask");
    if (typeof s_pageURL_mask !== "undefined") {
      s.pageURL = s.pageURL.replace(s_pageURL_mask, "mask");
    }
    s.referrer = s.referrer || document.referrer;
    s.referrer = s.referrer.replace(/(username=[^&]*|skype-session=[^&]*)/g, "mask");
    if (typeof s_referrer_mask !== "undefined") {
      s.referrer = s.referrer.replace(s_referrer_mask, "mask");
    }
  } catch (e) { }
...
if ((typeof s_allowSurvey !== "undefined") &&
  (s_allowSurvey === true)) {
  s.loadModule("Survey");
...
s.trackPage = function (_t) {
  var _r, key,
    args = _t;
  if (typeof s_el !== "undefined" &&
    s.el.findImpressionData(s_el)) {
    args = s.el.mergeImpressionData(args);
  }
  for (key in args) {
    s[key] = args[key];
  }
...
s.trackAction = function (_args, el) {
  var args = _args || {};
  args.pageName = (args.pageName || s.pageName || "");
  args.eVar7 = ((args.pageName && "D=pageName") || "");
  args.prop5 = (args.prop5 || s.prop5 || "");
  args.eVar5 = (s.prop5 && "D=c5") || "";
  args.prop54 = (args.prop54 || s.prop54 || "");
  args.eVar54 = (s.prop54 && "D=c54") || "";
  args.channel = (args.channel || s.channel || "");
  args.eVar15 = (s.channel && "D=ch") || "";
  return s.tl2(args, el);
};

s.tl2 = function (args, el) {
  var _r, key;
  ...
  for (key in args) {
    s[key] = args[key];
    s.linkTrackVars = s.linkTrackVars.replace("None", "");
    s.linkTrackVars += key + ',';
    if (key.indexOf('events') !== -1) {

      s.linkTrackEvents = s.linkTrackEvents.replace("None", args[key].split(":")[0]);
    }
  }
  ...
  if ((typeof el !== "undefined") &&
    ...
    _r = s.tl(el, 'o', 'trackAction', null, 'navigate');
  } else {
    _r = s.tl(true, 'o', 'trackAction');
  }
...
var s = s || {};
s.el = s.el || {};
s.el.findAttributesUp = function (el, attribute) {
  var attr = [];
  while (el) {
...
var s = s || {};
s.el = s.el || {};
s.el.getClickData = function (el, json) {
  var _i,
    _json = json || s_el || null,
    _attrs = s.el.findAttributesUp(el, "data-sc"),
    _data;
...
var s = s || {};
s.el = s.el || {};
s.el.track = function (el) {
  var _t = s.el.getClickData(el),
    _hasData = (typeof _t !== "undefined"),
    _asPage = (_hasData && _t.asPage) || false;

  return (_hasData && _asPage && s.trackPage(_t)) ||
    (_hasData && !_asPage && s.trackAction(_t, el));
};

var s = s || {};
s.el = s.el || {};
s.el._impressions = s.el._impressions || [];
s.el.findImpressionData = function (s_el) {
  var _i,
    _data = s_el || {};
  for (_i in _data) {
    if (_i === "impression" &&
      (typeof _data[_i].track === "undefined" ||
        (typeof _data[_i].track !== "undefined" &&
          _data[_i].track === true))) {
      delete _data[_i].track;
      s.el._impressions.push(_data[_i]);
      _data[_i].track = false;
    }
    if (_data[_i] instanceof Object) {
      s.el.findImpressionData(_data[_i]);
    }
  }
  return true;
};
...
var s = s || {};
s.el = s.el || {};
s.el.impressions = s.el.impressions || [];
s.el.mergeImpressionData = function (_t) {
  var _i, _var,
    _impressions = s.el._impressions || [],
    _data = _t || {};


  for (_i in _impressions) {
    for (_var in _impressions[_i]) {
      if (_data[_var] && (_var === "products" || _var === "events")) {
        _data[_var] = s.apl(_data[_var], _impressions[_i][_var], ',', 1);
      } else {
        _data[_var] = _impressions[_i][_var];
      }
    }
  }
