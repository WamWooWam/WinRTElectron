...
function receiveMessage(msg) {
    ...
    if (msgType === MSG_TYPE_SCRIPT) {
        ...
        try {
            ...
            var element = document.getElementById('adWarning');
            if (element) {
                 element.removeNode(true);
            }
			var isSkypeAd = (location.search.match(/isSkypeAd=1/ig) !== null);
			if (isSkypeAd) {
				var clickEl = document.getElementsByClassName('ctLink');
				if (clickEl && clickEl.length === 1) {
					clickEl[0].title = "";
				}
			}
...
if (bubblePointerEvents) {
    document.body.addEventListener("MSPointerDown",
        function (evt) {
            pointerDown = true;
            postToLocal(MSG_TYPE_ONPOINTERDOWN + ":" + JSON.stringify({ clientX: evt.clientX, clientY: evt.clientY }));
        },
        false
    );
    document.body.addEventListener("MSPointerUp",
        function (evt) {
            pointerDown = false;
    ...
    document.body.addEventListener("MSPointerCancel",
        function (evt) {
            pointerDown = false;
            postToLocal(MSG_TYPE_ONPOINTERUP);
        },
    ...
    document.body.addEventListener("MSPointerOut",
        function (evt) {
            if (pointerDown) {
                pointerDown = false;
                postToLocal(MSG_TYPE_ONPOINTERUP);
    ...
    document.body.addEventListener("MSLostPointerCapture",
        function (evt) {
            pointerDown = false;
            postToLocal(MSG_TYPE_ONPOINTERUP);
        },
