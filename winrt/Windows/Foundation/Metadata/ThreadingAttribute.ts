// --------------------------------------------------
// <auto-generated>
//     This code was generated by tswinrt.
//     Generated from Windows 255.255.255.255 at Fri Mar 26 17:23:03 2021
// </auto-generated>
// --------------------------------------------------

import { GenerateShim } from "../Interop/GenerateShim";
import { ThreadingModel } from "./ThreadingModel";

@GenerateShim('Windows.Foundation.Metadata.ThreadingAttribute')
export class ThreadingAttribute { 
    constructor(model: ThreadingModel) {
        console.warn('ThreadingAttribute.ctor not implemented')
    }
}
